...
def get_entities_from_docid_parid(cursor, docid: str, parid: str, column: str) -> list:
    """Get entity list for specific document from database."""
    cursor.execute(f'SELECT {column} FROM entities WHERE docid=? AND parid=?', (docid,parid,))
    entity_tuple_list = cursor.fetchall()
    entity_list = []
    for tup in entity_tuple_list:
        for entity in tup[0].split('\n'):
...
def get_parids_from_docid(cursor, doc_id: str) -> list:
    cursor.execute(
        "select parid from entities where docid=?;", (doc_id,))
