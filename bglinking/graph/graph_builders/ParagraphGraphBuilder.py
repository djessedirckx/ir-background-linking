...
class ParagraphGraphBuilder(InformalGraphBuilderInterface):
    def build(self, graph, cursor, embeddings, index_utils, docid, use_entities, nr_terms=0, term_tfidf=0.0, term_position=0.0, text_distance=0.0, term_embedding=0.0):
        ...
        if nr_terms > 0.0:
            par_ids = db_utils.get_parids_from_docid(cursor, docid)
            for par_id in par_ids:
                fname = f'sub_graph_{docid}_{par_id}'
                sub_graph = Graph(docid, fname, self)

                terms = db_utils.get_entities_from_docid_parid(
                    cursor, docid, par_id, 'tfidf_terms')[:nr_terms]
                ...
                equalize_term_and_entities(sub_graph)

                embeddings_not_found = 0
                # Initialize edges + weights
                for node_key in sub_graph.nodes.keys():
                    for other_node_key in sub_graph.nodes.keys():
                        if node_key == other_node_key:
                            continue
                        weight = 0.0
                        if text_distance > 0:
                            distance = closest_distance(
                                sub_graph.nodes[node_key], sub_graph.nodes[other_node_key])
                            weight += text_distance * distance_in_text(distance)

                        if term_embedding > 0:
                            weight += term_embedding * edge_embedding_weight(
                                sub_graph.nodes[node_key], sub_graph.nodes[other_node_key], embeddings, embeddings_not_found)

                        if weight > 0.0:
                            sub_graph.add_edge(node_key, other_node_key, weight)

                sub_graph_list.append(sub_graph)
            return sub_graph_list, par_ids
